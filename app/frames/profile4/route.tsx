...
export const POST = frames(
  async (ctx: {
    clientProtocol: { id: string };
    message: { profileId: any };
  }) => {
    if (ctx.clientProtocol?.id === "lens") {
      ...
      // Iterate through result items and aggregate the revenue data
      revenueArray.items.forEach((item: { revenue: any[] }) => {
        item.revenue.forEach(
          (revenueItem: { total: { asset: any; value: any; asFiat: any } }) => {
            const { asset, value, asFiat } = revenueItem.total;

            // Aggregate asset revenue
            if (asset.symbol in assetRevenue) {
              assetRevenue[asset.symbol] += parseFloat(value);
            } else {
              assetRevenue[asset.symbol] = parseFloat(value);
            }

            // Aggregate fiat revenue
            if (asFiat) {
              totalFiatRevenue += parseFloat(asFiat.value);
            }
          }
        );
      });
      ...
      return {
        image: (
          <div tw="flex flex-col bg-[#121212] text-white p-5 w-full h-full items-center justify-center">
            <div tw="flex">
              <svg viewBox="0 0 88 58" xmlns="http://www.w3.org/2000/svg">
                <path
            ...
            <div tw="text-5xl pb-5">Publication Revenue</div>
            {Object.entries(assetRevenue).map(([symbol, total]) => (
              <div>{`${symbol}: ${total}`}</div>
            ))}
            <div tw="text-4xl pb-5">{`Total USD: $${totalFiatRevenue.toFixed(
              2
            )}`}</div>
          </div>
        ),
      ...
      return {
        image: (
          <div tw="flex p-5">
            App not compatible, Lens clientProtocol required in Frame request
          </div>
        ),
        buttons: [
          <Button target={"/"} action="post">
