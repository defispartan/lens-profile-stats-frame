...
function profileLifecyclePercentage(profileCreationDateString: string): string {
  const lensLaunchDate = new Date("2022-05-18T00:00:00.000Z");
  const profileCreationDate = new Date(profileCreationDateString);
  const today = new Date();

  const totalLifecycleTime = today.getTime() - lensLaunchDate.getTime();
  const profileExistedTime = today.getTime() - profileCreationDate.getTime();

  const percentage = (profileExistedTime / totalLifecycleTime) * 100;
  ...
  if (ctx.clientProtocol?.id === "lens") {
    ...
    return {
      ...
      buttons: [
        ...
        <Button target={"/profile2"} action="post">
          Back ⬅️
        </Button>,
